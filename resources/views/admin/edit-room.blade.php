...
<h1 class="mt-2">Edit Room</h1>
    <form method="POST" action="{{ route('update_room', $room->id) }}" enctype="multipart/form-data"  class="bg-dark p-4">
    @method('PUT')    
    @csrf
        <div class="mb-3">
            ...
            @enderror
            <input type="text" class="form-control bg-dark text-light border-secondary" autofill="title" name="title" value="{{ $room->title }}">
        </div>
        ...
        <div class="mb-3">
            ...
            autofill="description">{{ $room->description }}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            @error('rent')
                <div class="text-danger small mt-1">{{ $message }}</div>
            @enderror
            <input type="text" class="form-control bg-dark text-light border-secondary" value="{{ $room->rent }}" 
        ...
        <div class="mb-3 form-check">
            ...
            <input type="checkbox" class="form-check-input bg-dark border-secondary" value="yes" id="facility" 
                {{ $room->facility == 'yes' ? 'checked' : '' }} name="facility">
